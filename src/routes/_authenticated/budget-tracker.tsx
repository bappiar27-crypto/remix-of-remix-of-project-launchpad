  ...
  const { data: pacing } = useQuery({
    ...
    queryFn: async () => {
      ...
      const byClient: Record<
        string,
        { dates: Set<string>; total: number; perDay: Record<string, number> }
      > = {};
      (data ?? []).forEach((r: any) => {
        ...
        byClient[cid].perDay[r.date_start] =
          (byClient[cid].perDay[r.date_start] ?? 0) + (Number(r.spend) || 0);
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Monthly pacing & spend forecast based on the last 30 days of insights.
        </p>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {(clients ?? []).length === 0 ? (
          <div className="glass-card p-12 text-center md:col-span-2">
            <Wallet className="size-10 mx-auto opacity-30 mb-2" />
              ...
              <div key={c.id} className="glass-card p-5 space-y-3">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {c.currency} {spent.toFixed(2)} / {c.currency} {budget.toFixed(2)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1.5 text-xs text-muted-foreground flex justify-between">
                    ...
                    {budget > 0 && (
                      ...
                      >
                        {pct > expectedPct
                          ? `+${(pct - expectedPct).toFixed(1)}pp vs schedule`
                          : `${(expectedPct - pct).toFixed(1)}pp under schedule`}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <Tile
                    ...
                    accent={budget > 0 ? (onTrack ? "success" : "destructive") : undefined}
                  />
                </div>
                {budget > 0 && !onTrack && (
                  <div className="flex items-start gap-2 text-xs text-destructive bg-destructive/10 rounded-lg p-2.5">
                    <AlertTriangle className="size-3.5 mt-0.5 shrink-0" />
                    Projected to exceed budget by {c.currency} {overBy.toFixed(2)} (
