  ...
  const { data: snapshots } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select(
          "date_start,spend,clicks,impressions,reach,results,ad_account:ad_accounts(account_name,client:clients(name))",
        )
        .eq("level", "account")
    ...
    try {
      ...
      if (level === "timeseries") {
        const { data } = await supabase
          .from("insights_snapshots")
          .select(
            "date_start,date_stop,spend,reach,impressions,clicks,ctr,cpc,cpm,results,frequency,ad_account:ad_accounts(account_name,fb_account_id,client:clients(name))",
          )
          .eq("level", "account")
        ...
        const { data } = await supabase
          .from(table)
          .select(
            "name,effective_status,spend,reach,impressions,clicks,ctr,cpc,cpm,results,frequency,ad_account:ad_accounts(account_name,fb_account_id,currency,client:clients(name))",
          )
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            30-day time-series across all accounts. Auto-synced from Facebook Marketing API.
          </p>
        </div>
