...
import {
  ...
  saveSettings,
  testFbToken,
  importVisibleAdAccounts,
  checkTokenHealthNow,
  detectBusinessesFromToken,
...
export function LegacySettingsSection() {
  ...
  const onDetect = async () => {
    const r = await run(
      "detect",
      () => detectFn({ data: undefined as any }),
      ...
      {open && (
        <div className="mt-4 space-y-4">
          <p className="text-xs text-muted-foreground">
            পুরানো single-BM setup এখনো কাজ করছে। নতুন BM যোগ করতে চাইলে উপরের "Business Managers"
            section ব্যবহার করুন — multi-BM সবসময় সঠিক token দিয়ে sync করবে।
          </p>
          ...
          {s?.token_status && (
            <div className="text-xs flex items-center gap-2 text-muted-foreground">
              ...
              Token status:{" "}
              <span className={s.token_status === "ok" ? "text-success" : "text-warning"}>
                {s.token_status}
              </span>
              {s.token_user_name && (
                <>
                  · user: <span className="text-foreground">{s.token_user_name}</span>
                </>
              )}
              {s.token_checked_at && (
                <>
                  ...
                  <span className="text-foreground">
                    {new Date(s.token_checked_at).toLocaleString()}
                  </span>
                </>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            <button
              onClick={() =>
                run(
                  "health",
                  () => healthFn({ data: undefined as any }),
                  (r) => `Status: ${r?.status}`,
                )
              }
              disabled={!!busy}
            ...
            <button
              onClick={() =>
                run(
                  "import",
                  () => importFn({ data: undefined as any }),
